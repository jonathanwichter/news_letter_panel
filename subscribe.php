...
$token = \Defender\Token::generate_token($options['form_id'], $options['max_tokens']);
		echo "<form id='".$options['form_id']."' name='nl_sub' method='post' action='".FUSION_SELF."?step=subscribe'>\n";
...
$options['max_tokens'] = "10";
$token = \Defender\Token::generate_token($options['form_id'], $options['max_tokens']);
		echo "<form id='".$options['form_id']."' name='nl_unsub' method='post' action='".FUSION_SELF."?step=unsubscribe'>\n";
		echo "<input type='hidden' name='fusion_token' value='".$token."' />\n";
		echo "<input type='hidden' name='form_id' value='".$options['form_id']."' />\n";
	...
	if (isset($_POST['unsubscribe'])) {
		...
		if (!isset($_POST['subscribe']) AND !isset($_POST['unsubscribe'])) {
			echo "<div align='center'><div style='text-align:center'>".$locale['nl_491']."</div><br />";
		}
$options['form_id'] = rand(100000,900000);
$options['max_tokens'] = "10";
$token = \Defender\Token::generate_token($options['form_id'], $options['max_tokens']);
	echo "<form id='".$options['form_id']."' name='sub_unsub' method='POST' action='".FUSION_SELF."'>\n";
	echo "<input type='hidden' name='fusion_token' value='".$token."' />\n";
	echo "<input type='hidden' name='form_id' value='".$options['form_id']."' />\n";
	echo "<input type='text' class='textbox' name='mail_addr' value='".$locale['nl_477']."'  onBlur=\"if(this.value=='') this.value='".$locale['nl_477']."';\" onFocus=\"if(this.value=='".$locale['nl_477']."') this.value='';\" size='25px' />\n";
	echo "<input type='text' class='textbox' name='mail_name' value='".$locale['nl_492']."'  onBlur=\"if(this.value=='') this.value='".$locale['nl_492']."';\" onFocus=\"if(this.value=='".$locale['nl_492']."') this.value='';\" size='25px' />\n";
	echo "<input type='submit' name='subscribe' value='".$locale['nl_472']."' class='button' />\n";
	echo "<input type='submit' name='unsubscribe' value='".$locale['nl_475']."' class='button' />\n";
	$data = dbarray(dbquery("SELECT COUNT(nl_sub_mail) as sub_no FROM ".DB_NEWS_LETTER_SUBS.""));
